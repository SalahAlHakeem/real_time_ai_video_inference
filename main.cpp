...
static bool create_source_bin(DsSourceBinStruct *ds_source_struct,  std::string str_uri)
{
    gchar bin_name[16] = {};
    GstCaps *caps = NULL;
    ...
    // Create elements: rtsp-source, nvvideoconvert, capsfilter
    ...
    g_object_set(G_OBJECT(ds_source_struct->uri_decode_bin), "uri", str_uri.c_str(), NULL);
...
int main(int argc, char *argv[])
{
    ...
    g_object_set(G_OBJECT(tracker),
            ...
            "ll-config-file", app_config["tracker"]["ll-config-file"].as<std::string>().c_str(),
            NULL
          );

    auto input_sources = app_config["sources"];

    for (guint current_source_index = 0; current_source_index < input_sources.size(); current_source_index++, src_cnt++) {
        source_struct[current_source_index].index = current_source_index;
        if (!create_source_bin(&(source_struct[current_source_index]), input_sources[current_source_index].as<std::string>() ))
        {
            g_printerr("Source bin could not be created. Exiting.\n");
            return -1;
        }

        gst_bin_add(GST_BIN(pipeline), source_struct[current_source_index].source_bin);
        g_snprintf(pad_name_sink, 64, "sink_%d", current_source_index);
        ...
        srcpad = gst_element_get_static_pad(source_struct[current_source_index].source_bin, pad_name_src);
        if (!srcpad)
        {
    ...
    cout << "source playing: " << src_cnt <<  endl;
    tiler_rows = (guint)sqrt(src_cnt);
    tiler_columns = (guint)ceil(1.0 * src_cnt / tiler_rows);
    g_object_set(G_OBJECT(nvtile), "rows", 2, "columns", 2, "width", 1280, "height", 720, NULL);
